      ...
      <div className="flex flex-col gap-4 w-full p-6 pb-4 bg-[#fbfbfb] rounded-t-[20px] max-h-[90vh] overflow-y-auto shadow-[-10px_25px_rgba(0,0,0,0.1)] animate-slideInUp dark:bg-brand-dark-bg dark:shadow-[-10px_25px_rgba(0,0,0,0.3)]">
        ...
        <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl dark:bg-brand-dark-surface dark:border-slate-700">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="langSelect" className={labelClass}>{t.lang_label}</label>
            <select id="langSelect" className={selectClass} value={settings.extensionLanguage} onChange={e => updateSettings({ extensionLanguage: e.target.value })}>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="engineSelect" className={labelClass}>{t.engine_label}</label>
            <select id="engineSelect" className={selectClass} value={settings.engine} onChange={e => updateSettings({ engine: e.target.value }, postMessage)}>
          ...
          {(settings.engine === 'local-gpu' || settings.engine === 'local-wasm') && (
            <div className="flex flex-col gap-1.5 overflow-visible">
              <label id="modelSelectLabel" className={labelClass}>{t.model_label}</label>
        ...
        <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl dark:bg-brand-dark-surface dark:border-slate-700">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="toneSelect" className={labelClass}>{t.tone_label}</label>
            <select id="toneSelect" className={selectClass} value={settings.tone} onChange={e => updateSettings({ tone: e.target.value })}>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="detailSelect" className={labelClass}>{t.detail_label}</label>
            <select id="detailSelect" className={selectClass} value={settings.detailLevel} onChange={e => updateSettings({ detailLevel: e.target.value })}>
