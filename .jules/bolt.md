...
## 2026-03-07 - [Single-pass Regex for Prompt Sanitization]
**Learning:** In string processing tasks like `formatUserPrompt` that sanitize structural tags (e.g. `<TEXT_TO_PROCESS>`), chained `.replace()` calls with global regular expressions cause multiple inefficient full-string traversals. This CPU overhead grows linearly with text length.
**Action:** Replace chained `.replace()` calls with a single-pass, hoisted compiled regular expression using alternation (`|`) to reduce full-string traversals and CPU overhead during text sanitization.
