...
export class EngineManager {
  ...
  async getAvailableEngines(): Promise<TranslationEngine[]> {
    ...
    // ⚡ Bolt: Use Promise.all to parallelize independent engine availability checks
    // This provides a significant performance improvement (e.g., ~5x speedup for 5 engines)
    await Promise.all(
  ...
  async getEngineInfos(): Promise<EngineInfo[]> {
    ...
    // ⚡ Bolt: Use Promise.all to parallelize independent engine availability checks
    // This significantly reduces the total time to get all engine infos
