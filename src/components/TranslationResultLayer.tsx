...
export function TranslationResultLayer({
  ...
  // 计算浮层位置（确保在可视区域内）
  // ⚡ Bolt: Cache position calculation to prevent unnecessary work on every render during text streaming
  // Impact: Eliminates ~30 recalculations per second during streaming
  const { x, y } = useMemo(() => {
    if (typeof window === 'undefined') return { x: position.x, y: position.y };
