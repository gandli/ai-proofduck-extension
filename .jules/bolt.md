...
## 2026-03-06 - [Hoist and Combine Regexes for String Sanitization]
**Learning:** Chaining multiple `.replace()` calls with global case-insensitive regular expressions causes the JavaScript engine to iterate over the entire string multiple times and dynamically recompile the regular expressions on every execution. In functions that process potentially large input texts (like `formatUserPrompt`), this creates measurable CPU overhead.
**Action:** Always combine related structural tag removals into a single regular expression using alternation (`|`), and hoist the definition outside of the function so it is only compiled once during module initialization.
