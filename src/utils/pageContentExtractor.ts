...
export function extractTextNodes(root: ParentNode = document.body): TextNodeInfo[] {
  ...
  // Cache to prevent redundant style recalcs for text nodes sharing the same parent
  const styleCache = new WeakMap<Element, { isHidden: boolean }>();
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode: (node: Node) => {
        ...
        let cached = styleCache.get(parent);
        if (!cached) {
          const style = window.getComputedStyle(parent);
          cached = { isHidden: style.display === 'none' || style.visibility === 'hidden' };
          styleCache.set(parent, cached);
        }
  ...
  // Cache bounding rects per parent as well
