...
class EdgeTTSProvider {
  ...
  /**
   * 生成 GUID
   * Security enhancement: Use crypto API for secure random UUID generation
   * with fallback for non-secure HTTP contexts (like content scripts).
   */
  private generateGuid(): string {
    ...
    if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') {
      ...
      return [...array].map(b => b.toString(16).padStart(2, '0')).join('').replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, '$1-$2-$3-$4-$5');
    }
