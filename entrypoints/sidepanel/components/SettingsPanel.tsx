      ...
      <div className="flex flex-col gap-4 w-full p-6 pb-4 bg-[#fbfbfb] rounded-t-[20px] max-h-[90vh] overflow-y-auto shadow-[-10px_25px_rgba(0,0,0,0.1)] animate-slideInUp dark:bg-brand-dark-bg dark:shadow-[-10px_25px_rgba(0,0,0,0.3)]">
        ...
        <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl dark:bg-brand-dark-surface dark:border-slate-700">
          ...
          <div className="flex flex-col gap-1.5">
            <label className={labelClass} htmlFor="extensionLanguage">{t.lang_label}</label>
            <select id="extensionLanguage" className={selectClass} value={settings.extensionLanguage} onChange={e => updateSettings({ extensionLanguage: e.target.value })}>
          ...
          <div className="flex flex-col gap-1.5">
            <label className={labelClass} htmlFor="engine">{t.engine_label}</label>
            <select id="engine" className={selectClass} value={settings.engine} onChange={e => updateSettings({ engine: e.target.value }, postMessage)}>
        ...
        {settings.engine === 'online' && (
          <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl dark:bg-brand-dark-surface dark:border-slate-700">
            ...
            <div className="flex flex-col gap-1.5">
              <label className={labelClass} htmlFor="apiBaseUrl">API Base URL</label>
            ...
            <div className="flex flex-col gap-1.5">
              <label className={labelClass} htmlFor="apiKey">API Key</label>
              <input id="apiKey" ref={apiKeyInputRef} className={inputClass} type="password" value={settings.apiKey} onChange={e => updateSettings({ apiKey: e.target.value })} aria-describedby="apiKeyHint" />
              <small id="apiKeyHint" className="text-[11px] text-slate-400">{t.api_key_hint || 'Get a free key from OpenRouter or your preferred OpenAI-compatible provider.'}</small>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className={labelClass} htmlFor="apiModel">Model ID</label>
        ...
        <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl dark:bg-brand-dark-surface dark:border-slate-700">
          ...
          <div className="flex flex-col gap-1.5">
            <label className={labelClass} htmlFor="tone">{t.tone_label}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className={labelClass} htmlFor="detailLevel">{t.detail_label}</label>
            <select id="detailLevel" className={selectClass} value={settings.detailLevel} onChange={e => updateSettings({ detailLevel: e.target.value })}>
          ...
          <div className="flex flex-col gap-1.5">
            <label className={labelClass} htmlFor="translateFallback">{t.translate_fallback_label || 'Translation Fallback'}</label>
            <select id="translateFallback" className={selectClass} value={settings.translateFallback || 'google-free'} onChange={e => updateSettings({ translateFallback: e.target.value as any })}>
