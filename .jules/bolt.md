...
## 2024-05-15 - [Single-Pass Alternation Regex vs Chained Replace]
**Learning:** Chaining multiple `.replace()` calls with simple regular expressions to sanitize or process strings forces the engine to traverse the string multiple times. Moreover, defining these regular expressions inline causes them to be recompiled on every function call.
**Action:** Use a single hoisted regular expression with alternation (`|`) to allow the engine to process the string in a single pass, and define the regex outside of the function to avoid recompilation overhead.
