...
## 2026-03-03 - Focus Visible alongside Outline None
**Learning:** Using Tailwind's `focus:outline-none` on interactive elements like custom combobox triggers entirely removes the browser's default focus ring. Without a corresponding custom focus indicator, keyboard users lose track of focus, creating a critical accessibility bug.
**Action:** Always pair `focus:outline-none` with explicit custom focus styles (e.g., `focus-visible:ring-2 focus-visible:ring-brand-orange/50`) to ensure keyboard navigability is maintained.
