...
describe('OpenAiCompatSection', () => {
  ...
  it('首次挂载显示 skeleton，随后回填已存储的值', async () => {
    ...
    const keyInput = (screen.getAllByLabelText(/API Key/i)[0] as HTMLElement) as HTMLInputElement;
  ...
  it('修改后点"保存" → 调用 set 且显示"已保存"', async () => {
    ...
    await user.type((screen.getAllByLabelText(/API Key/i)[0] as HTMLElement), 'sk-test');
  ...
  it('点击 👁️ 切换 API Key 显隐（password ↔ text）', async () => {
    ...
    render(<OpenAiCompatSection />);
    await waitFor(() => expect((screen.getAllByLabelText(/API Key/i)[0] as HTMLElement)).toHaveValue('sk-secret'));
