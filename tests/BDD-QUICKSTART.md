...
当前自动化已经覆盖多条真实浏览器场景，重点看这 6 类：

1. 页内 `🐣 / 🐥` 悬停翻译、点击送入侧边栏、卡片关闭
2. 侧边栏 `导入选区`、`抓取全文`、五种处理模式、官方模型推荐
3. 设置切换、引擎切换、自动回退
4. 浏览器 AI 路线
5. 本地模型 `GPU / 兼容模式` 路线，以及“侧边栏已打开时页内卡片直接复用本地模型”
6. 失败提示是否清楚，且不会冲掉原内容

如果你想额外确认两件事：

1. 真实本地 GPU 主链路：`bun run test:real`
2. 这台机器上的 Chrome 会不会拦扩展页：`bun run test:chrome-probe`
