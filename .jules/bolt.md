...
## 2026-03-08 - [Optimize formatUserPrompt Regex Usage]
**Learning:** Chaining multiple `.replace()` calls with inline regular expressions causes the string to be traversed multiple times and recompiles the regexes repeatedly on every execution. In functions that run frequently, such as prompt formatting logic prior to LLM generation, this creates unnecessary overhead.
**Action:** Use a single, hoisted, pre-compiled regular expression with alternation (`|`) instead of chaining `.replace()` calls to reduce full-string traversals and CPU overhead.
