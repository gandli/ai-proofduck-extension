...
## 2024-06-18 - [Optimize String Processing with Single-Pass Regex]
**Learning:** Chaining multiple `.replace()` calls with different regular expressions (e.g., `text.replace(/A/g, '').replace(/B/g, '')`) forces the engine to traverse the entire string multiple times and recompile regexes if declared inline. This causes unnecessary CPU overhead, especially for larger strings or frequently called utility functions like `formatUserPrompt`.
**Action:** When sanitizing strings or applying multiple string replacements, combine the patterns using alternation (`|`) into a single, hoisted, compiled regular expression (e.g., `const REGEX = /A|B/g; text.replace(REGEX, '')`) to ensure a single-pass traversal and prevent recompilation.
