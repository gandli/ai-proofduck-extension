...
export class EngineManager {
  ...
  async getAvailableEngines(): Promise<TranslationEngine[]> {
    // Optimization: Parallelize availability checks to prevent N+1 sequential bottlenecks.
    // Expected impact: Speeds up finding available engines from O(N*t) to O(t), where N is engines and t is max check time.
  ...
  async getEngineInfos(): Promise<EngineInfo[]> {
    // Optimization: Parallelize getting engine infos to prevent N+1 sequential bottlenecks.
    // Expected impact: Reduced total fetch time by running I/O or availability checks concurrently.
    const engines = Array.from(this.engines.values());
