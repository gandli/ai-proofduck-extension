...
export class EngineManager {
  ...
  async getAvailableEngines(): Promise<TranslationEngine[]> {
    const engineChecks = Array.from(this.engines.values()).map(async (engine) => {
      try {
    ...
    const results = await Promise.all(engineChecks);
    const available = results.filter((e): e is TranslationEngine => e !== null);
