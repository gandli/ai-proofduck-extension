...
## 2026-03-07 - [Hoist and Combine Regex in String Processing Utilities]
**Learning:** In text processing functions (like prompt sanitization `formatUserPrompt`), using multiple inline `.replace()` calls with statically defined regexes causes redundant string traversal passes and regex recompilation.
**Action:** Always combine disjoint replacements using alternation (`|`) and hoist the compiled regex outside of the function into module scope. This optimizes O(N) traversal passes to 1 and avoids CPU overhead from recompilation.
