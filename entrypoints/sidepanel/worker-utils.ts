...
// Hoist regular expression to avoid recompilation and use alternation for single-pass replacement
const SANITIZE_TAGS_REGEX = /<TEXT_TO_PROCESS>|<\/TEXT_TO_PROCESS>|\[FINAL RESULT\]:/gi;
...
export function formatUserPrompt(text: string, mode: string, targetLang: string): string {
    ...
    // Single-pass string traversal using alternation reduces CPU overhead
    const sanitizedText = text.replace(SANITIZE_TAGS_REGEX, '');
