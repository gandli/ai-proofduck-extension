...
// Hoisted regex for prompt sanitization to avoid recompilation and multiple traversals
const STRUCTURAL_TAGS_REGEX = /<\/?TEXT_TO_PROCESS>|\[FINAL RESULT\]:/gi;
...
export function formatUserPrompt(text: string, mode: string, targetLang: string): string {
    ...
    const sanitizedText = text.replace(STRUCTURAL_TAGS_REGEX, '');
